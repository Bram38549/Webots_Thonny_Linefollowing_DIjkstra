...
class PathPlannerESP32:
    def __init__(self):
        ...
        self.current_node = "P1"  # Changed to P1 as the starting position
    ...
    def state_machine(self, gs_values, ps_values, obstacle_detected):
        ...
        if obstacle_detected:
            if self.current_state != 'obstacle_avoidance' and self.current_state != 'turning_180':
                print("Obstacle detected! Initiating 180-degree turn for replanning...")
            ...
            # If already in turning_180, continue the turn
                ...
                if turn_complete:
                    print("180-degree turn complete. Replanning path...")
                    ...
                    if node_robot_was_trying_to_reach:
                        ...
                        if temp_new_path:
                            ...
                            print(f"New path constructed as per user request: {' -> '.join(self.path)}")
                            ...
                            print("No new path found from current node after obstacle. Stopping.")
                            self.current_state = 'stopping'
                    else:
                        print("Error: Could not determine node for replanning. Stopping.")
                        self.current_state = 'stopping'
        ...
        if self.current_state == 'line_following':
            if self.detect_intersection(gs_values):
                ...
                if self.intersection_counter >= 3 and not self.intersection_processed:
                    print(f"Intersection detected! Moving from {self.current_node} towards {self.path[self.next_node_index]}")
                    ...
                    if self.next_node_index < len(self.path):
                        ...
                        print(f"Robot now logically at node: {self.current_node}")
                    ...
                    self.intersection_counter = 12 # Set delay cycles for at_intersection state (e.g., 8 -> 12)
            ...
            if self.intersection_counter > 0:
                ...
                if self.next_node_index >= len(self.path):
                    ...
                    if direction:
                        ...
                        print(f"ERROR: No direction found from {self.current_node} to {next_target}")
                        self.current_state = 'stopping'
